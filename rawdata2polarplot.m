...
image_size = 401;
v_scale = linspace(-v_max, v_max, image_size);
r_max = max(v_range)/v_max;
r_min = (v_centre + min(v_range)) / v_max;

rawdata_plot=[rawdata, rawdata];
warning off
raw_polar = PolarToIm(rawdata_plot,r_min,r_max,image_size,image_size);
warning on

raw_polar = rot90(raw_polar);
raw_polar(:,image_size-floor(image_size/2)+1:end) = rot90(raw_polar(:,1:floor(image_size/2)),2);
raw_polar = rot90(raw_polar);
